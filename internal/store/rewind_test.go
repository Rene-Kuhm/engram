...
// TestRewindSession_TruncateMode_NotImplemented covers REQ-011 in PR2:
// truncate mode is RESERVED for PR4. In PR2 it MUST return an error and
// MUST NOT mutate any rows. We test both:
//   - Mode=truncate without ConfirmTruncate → ErrInvalidRewindMode (per spec)
//   - Mode=truncate with ConfirmTruncate=true → also ErrInvalidRewindMode
//     (the path is stubbed in PR2; PR4 will gate confirm as well)
func TestRewindSession_TruncateMode_NotImplemented(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()

	// Seed 3 turns.
	const sid = "trunc-stub"
	t0, err := s.SaveTurn(ctx, SaveTurnParams{
		SessionID: sid, Project: "proj-X", Role: "user",
	...
	p1 := t0.ID
	t1, err := s.SaveTurn(ctx, SaveTurnParams{
		SessionID: sid, Project: "proj-X", Role: "assistant",
		ParentTurnID: &p1, ContentJSON: []byte(`[{"type":"text","text":"t2"}]`),
	})
	if err != nil {
	...
	if _, err := s.SaveTurn(ctx, SaveTurnParams{
		SessionID: sid, Project: "proj-X", Role: "user",
		ParentTurnID: &p2, ContentJSON: []byte(`[{"type":"text","text":"t3"}]`),
	}); err != nil {
	...
	cases := []struct {
		...
		{"truncate_without_confirm_rejected", false, ErrInvalidRewindMode},
		{"truncate_with_confirm_still_stubbed_in_pr2", true, ErrInvalidRewindMode},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := s.RewindSession(ctx, RewindSessionParams{
				...
				ConfirmTruncate: tc.confirmTruncate,
				FromProject:     "proj-X",
			})
			if !errors.Is(err, tc.wantErr) {
				t.Errorf("err = %v, want %v (REQ-011 PR2 stub)", err, tc.wantErr)
			}
		})
	}
	...
	if postCount != preCount {
		t.Errorf("truncate stub mutated rows: pre=%d, post=%d", preCount, postCount)
	}
}
