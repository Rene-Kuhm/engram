...
func (s *Store) migrate() error {
	...
	// ── Phase: jsonl-session-tree (PR1a/PR3) — session_turns table + indexes ──
	...
	// Locked-in decision C (PR3): UNIQUE(session_id, turn_seq) is the durable
	// guarantee against concurrent-save turn_seq collisions. The
	// SessionTurnRepository.Save retry policy (max 3 attempts with jittered
	// backoff) lands on top of this index. Idempotent via IF NOT EXISTS so
	// re-running migrate() on a v7 DB is a no-op.
	if _, err := s.execHook(s.db, `
		...
		CREATE UNIQUE INDEX IF NOT EXISTS uniq_session_turns_session_seq
			ON session_turns(session_id, turn_seq);
	`); err != nil {
		return err
	}
